...
##############################################################################
    ...
    #         })
    #     self.grating = MyGrating(self.experiment_configs)
    #     item = self.grating
...
##############################################################################
    ...
    def collect_raw_data(self):
        ...
        try:
            ...
            if val != '':
                self.experiment_configs['g_type'] = str(val)
            else:
                self.experiment_configs['g_type'] = 'SawTooth'
        except ValueError as e:
            message = 'Grating type must be a string'
            raise InputError(message, e)
        
        if self.experiment_configs['g_type'] == 'Custom':
            self.experiment_configs['max_display_x'] = 1920
            self.experiment_configs['max_display_x'] = 1152
            self.experiment_configs['grating_name'] = self.grating_name
            self.experiment_configs['file_path'] = self.grating_file_path
        else:
            #Rotation Angle
            try:
                #pdb.set_trace()
                ...
                if val != '':
                    self.experiment_configs['g_angle'] = int(val)
                else:
                    self.experiment_configs['g_angle'] = 0
                    #pdb.set_trace()
            except ValueError as e:
            ...
            try:
                ...
                if val != '':
                    self.experiment_configs['y_min'] = int(val)
                else:
                    self.experiment_configs['y_min'] = 0
            except ValueError as e:
            ...
            try:
                ...
                if val != '':
                    self.experiment_configs['y_max'] = int(val)
                else:
                    self.experiment_configs['y_max'] = 0
            except ValueError as e:
            ...
            try:
                ...
                if val != '' or val > 0:
                    self.experiment_configs['period'] = int(val)
                else:
                    self.experiment_configs['period'] = 100
            except ValueError as e:
                message = 'Period width (pixels) must be an int greater than 0'
                raise InputError(message, e)
            self.experiment_configs['reverse'] = self.g_reverse.get()
    ...
    def populate_main(self, datas:dict):
        ...
        for i in range(1,end):
            ...
            if 'Grating File %d' %(i) in datas:
                self.pattern_select(datas['Grating File %d' %(i)])
